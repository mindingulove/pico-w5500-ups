#!/usr/bin/env python3
"""
UPS Battery Monitor - Proxmox Backup Server shutdown agent
Polls the Pico W5500 HTTP endpoint, logs readings to SQLite, and shuts down
PBS gracefully when the battery is low, waiting for active backup tasks first.
...
Install:
    ...
    cp ups_monitor_pbs.py /usr/local/bin/ups_monitor_pbs.py
    cp ups_monitor_pbs.service /etc/systemd/system/
    systemctl enable --now ups_monitor_pbs
"""
...
LOG_FILE = os.environ.get("UPS_LOG_FILE", "/var/log/ups_monitor_pbs.log")
DB_PATH = os.environ.get("UPS_MONITOR_DB", "/var/lib/ups_monitor_pbs/ups_monitor_pbs.sqlite3")
...
TASK_WAIT_MAX = int(os.environ.get("UPS_TASK_WAIT_MAX", "780"))
...
def running_tasks():
    """Return list of active PBS task UPIDs (empty if none or tool missing)."""
    try:
        ...
        return [t for t in tasks if not t.get("endtime")]
    except Exception as e:
        log.warning("Could not query PBS tasks: %s", e)
        return []


def wait_for_tasks():
    """Wait up to TASK_WAIT_MAX seconds for running backup tasks to finish."""
    deadline = time.monotonic() + TASK_WAIT_MAX
    while time.monotonic() < deadline:
        active = running_tasks()
        if not active:
            log.info("No active backup tasks - safe to shut down.")
            return
        log.warning(
            "Waiting for %d active task(s) to finish (%.0fs remaining)...",
            len(active),
            deadline - time.monotonic(),
        )
        time.sleep(10)
    log.warning("Task wait timed out - proceeding with shutdown anyway.")


def do_shutdown():
    log.critical("Initiating PBS shutdown sequence.")
    wait_for_tasks()
    log.critical("Stopping proxmox-backup service...")
    subprocess.run(["systemctl", "stop", "proxmox-backup"], check=False)
    log.critical("Powering off.")
...
def main():
    log.info("PBS UPS monitor started. Polling %s every %ds", list(unique_urls()), POLL_INTERVAL)
        ...
        while not shutdown_requested.is_set():
            ...
            if raw_status is None:
                ...
                if fail_count >= FAIL_LIMIT:
                    log.error("Pico unreachable - NOT shutting down without data.")
                    fail_count = 0
